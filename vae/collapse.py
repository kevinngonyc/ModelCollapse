...
import os
import copy
from utils import device, load_vae, create_dataloader, combine_dataloader
from train import train_vae, vae_loss, train_dataset
...
def collapse(vae_init, vae_start, samples, loss, noise_input=False, noise_stdev=1, syn_ratio=1.0, debug=False):
...
def collapse_and_save(vae_init, vae, suffix, noise_input=False, noise_stdev=1, syn_ratio=1.0):
    samples = []
    loss = []
    collapse(vae_init, vae, samples, loss, noise_input, noise_stdev, syn_ratio)
...
if __name__=="__main__":
    ...
    vae_init = load_vae(os.path.join(Paths.model_dir, "vae_init.pt"))

    if sys.argv[1] in ["input", "no_noise", "synthetic"]:
        vae = vae_init
    elif sys.argv[1] == "latent":
        vae = load_vae(os.path.join(Paths.model_dir, "vae_latent.pt"))
    elif sys.argv[1] == "gradient":
        vae = load_vae(os.path.join(Paths.model_dir, "vae_gradient.pt"))
    else:
