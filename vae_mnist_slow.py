...
def train_vae_gen(vae, train_dataloader, sample_vae=False, sample_from=None, grad_noise=False):
    ...
    for epoch in range(num_epochs):
        ...
        for image_batch, _ in train_dataloader:
            ...
            loss.backward()
            if grad_noise:
                for p in vae.module.parameters():
...
def collapse(vae_start, samples, loss, noise_input=False, noise_stdev=1):
    ...
    for _ in range(20):
        ...
        img_recon = vae.module.decoder(latent).clone().detach()
...
if __name__ == '__main__':
    ...
    with ProcessPoolExecutor() as executor:
        ...
        vae_init = nn.DataParallel(VariationalAutoencoder()).to(device)
        v1 = executor.submit(train_vae_gen, vae_init, train_dataloader, False, None)

        vae_nl = nn.DataParallel(NoisyLatentVariationalAutoencoder()).to(device)
        v2 = executor.submit(train_vae_gen, vae_nl, train_dataloader, False, None)

        vae_ng = nn.DataParallel(VariationalAutoencoder()).to(device)
        v3 = executor.submit(train_vae_gen, vae_ng, train_dataloader, False, None, grad_noise=True)

        wait([v1, v2, v3])

        samples_no_noise = []
        loss_no_noise = []
        executor.submit(collapse_and_save, vae_init, samples_no_noise, loss_no_noise, "no_noise")

        samples_ni = [[] for _ in range(9)]
        loss_ni = [[] for _ in range(9)]
        stdev_ni = [10**(-i) for i in range(9)]
        for i in range(len(samples_ni)):
            executor.submit(collapse_and_save, vae_init, samples_ni[i], loss_ni[i], f"ni_{stdev_ni[i]}", True, stdev_ni[i])

        samples_nl = []
        loss_nl = []
        future = executor.submit(collapse_and_save, vae_nl, samples_nl, loss_nl, "nl")

        samples_ng = []
        loss_ng = []
        executor.submit(collapse_and_save, vae_ng, samples_ng, loss_ng, "ng")
